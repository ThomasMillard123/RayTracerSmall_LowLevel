...
void render(const std::vector<Sphere*> &spheres, int iteration)
{
	...
	int numberOfProcessecs = 2;
	int numberOfScreenSplit = 4;

	//create pipe
	int fd1[2];  
	if (pipe(fd1)==-1) { return; } 
	...
	if(pid==0){
		...
		Raytrace(pixel, spheres, height/numberOfScreenSplit, width, invWidth, invHeight, angle, aspectratio,0,0);
		Raytrace(pixel+ (height / numberOfScreenSplit) * width, spheres, (height/numberOfScreenSplit)*2, width, invWidth, invHeight, angle, aspectratio, (height / numberOfScreenSplit),0);
		
		//out data
		Vec3f inData[width*height];
		...
		close(fd1[0]);
		write(fd1[1],inData,sizeof(inData));
		close(fd1[1]);
		...
		Raytrace(pixel+ ((height / numberOfScreenSplit) * width)*2, spheres, (height / numberOfScreenSplit) * 3, width, invWidth, invHeight, angle, aspectratio, (height / numberOfScreenSplit)*2, 0);
		Raytrace(pixel+ ((height / numberOfScreenSplit) * width)*3, spheres, (height / numberOfScreenSplit) * 4, width, invWidth, invHeight, angle, aspectratio,(height / numberOfScreenSplit)*3,0);
	   
  	} 

	//recive data
		Vec3f outData[width*height];
  		close(fd1[1]);
		read(fd1[0],outData,sizeof(outData));
		close(fd1[0]);

		for(int i=0;i<(width*height)/numberOfProcessecs;i++,++pixel){
			*pixel =outData[i];
		}
